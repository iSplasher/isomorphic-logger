...
import {
  ...
  getLogLevelName,
} from "../../../src/processors/createDateAndLevelPrependProcessor";
...
describe(`getLogLevelName`, () => {
  ...
  it(`prepends level and and current date to record.message`, () => {
    const dateAndLevelPrepender = createDateAndLevelPrependProcessor({
      dateFormat: "YYYY",
    });
    const records = dateAndLevelPrepender([
      { level: LogLevel.TRACE, messages: ["foo"] },
    ]);
