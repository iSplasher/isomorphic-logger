...
import {
  ...
  defaultTestMessage,
} from "../../../src/processors/createErrorWrapProcessor";
...
describe(`createErrorWrapProcessor`, () => {
  ...
  it(`trims error stack depending on trimHeadFrames option`, () => {
    const errorWrapperWithoutTrim = createErrorWrapProcessor({
      trimHeadFrames: 3,
      loggerStackFrameCount: 0,
    });
    const errorWrapperWithTrim = createErrorWrapProcessor({
      trimHeadFrames: 5,
      loggerStackFrameCount: 0,
    });
    const records = [{ level: LogLevel.ERROR, messages: ["error message"] }];
    const stackA =
      errorWrapperWithoutTrim(records)[0].messages[0].stack.split("\n");
    const stackB =
      errorWrapperWithTrim(records)[0].messages[0].stack.split("\n");
...
describe(`defaultCreateStackTrace`, () => {
  it(`creates a stack trace from type, message and stack frames with function name`, () => {
    expect(
      defaultCreateStackTrace("Type Error", "Unexpected Token", [
        {
          fileName: "a.js",
          lineNumber: "10",
          columnNumber: "20",
