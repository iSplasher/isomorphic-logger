import {
  ...
  stringifyLog,
} from "../../../src/processors/createMessageConcatProcessor";
...
describe(`createMessageConcatProcessor`, () => {
  ...
  it(`returns concatenated string message and meta for multiple arguments`, () => {
    ...
    const records = [
      {
        level: "info",
        messages: ["Uncaught", error, "on page with following params", object],
      },
    ];
    expect(logsConcatenator(records)[0]).toEqual({
      level: "info",
      messages: [
        `Uncaught exception (#1) on page with following params {"foo":"bar"} (#2)`,
      ],
      meta: {
        "#1": error,
        "#2": object,
