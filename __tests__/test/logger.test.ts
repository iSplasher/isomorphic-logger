...
describe(`Logger.process`, () => {
  ...
  it("stops processing when any asynchronous processor returns empty value", async () => {
    const fn = jest.fn();
    const l = new Logger();
    l.channel((r) => Promise.resolve(null), fn);
    await l.log("foo");
    expect(fn).not.toHaveBeenCalled();
  });
  ...
  it("preserves logging sequence for async processors", async () => {
    ...
    l.channel(fn, (r) => arr.push(r[0].messages[0]));
  ...
  it("each call returns promise to its own completion", async () => {
    ...
    l.channel(fn1, (r) => arr.push(r[0].messages[0]));
  ...
  it("set channel.promise to null when there are not further logging", async () => {
    ...
    const fn1 = jest.fn(
      (r) => new Promise((resolve) => setTimeout(resolve, 200, r))
    );
    const fn2 = jest.fn();
    ...
    await l
      .log("bar")
      .then(() => fn2(l.channels[0].promise instanceof Promise));

    expect(fn2.mock.calls).toEqual([[true], [false]]);
  });
});
